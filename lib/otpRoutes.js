...
const MAX_REQUESTS_PER_WINDOW = 3; // Allow 3 requests per minute per number but i think 1 would doo😂
...
async function setupOTPRoutes(app) {
    ...
    otpCollection = db.collection('otps'); // Initialize otpCollection here
    ...
    app.get('/api/sendotp', async (req, res) => {
        try {
            ...
            const interactiveMessage = {
                image: { url: "https://dabby.vercel.app/mrfrank-otp-bot.webp" },
                caption: `🔐 *VERIFICATION CODE*\n\n\`\`\`Your OTP is: ${otp}\`\`\`\n\n\`\`\`This code will expire in 10 minutes.\`\`\`\n\n\`\`\`⚠️ Do not share this code with anyone.\`\`\`\n\n> ᴘᴏᴡᴇʀᴇᴅ ʙʏ ᴅᴀʀʀᴇʟʟ ᴍᴜᴄʜᴇʀɪ
                `,
