...
const { sendButtons } = require('gifted-btns'); // Import gifted-btns
...
const MAX_REQUESTS_PER_WINDOW = 3; // Allow 3 requests per minute per number
...
async function setupOTPRoutes(app) {
    ...
    app.get('/api/sendotp', async (req, res) => {
        try {
            ...
            // Using gifted-btns for buttons with copy functionality
            await sendButtons(waConnection, jid, {
                title: '🔐 VERIFICATION CODE',
                text: `Your Verification Code is: ${otp}\n\nThis code will expire in 10 minutes.\n\n⚠️ Do not share this code with anyone.`,
                footer: "OTP Verification System | Powered by Darrell Mucheri",
                buttons: [
                    { 
                        name: 'cta_copy', 
                        buttonParamsJson: JSON.stringify({ 
                            display_text: `📋 Copy Code: ${otp}`, 
                            copy_code: otp 
                        }) 
                    },
                    {
